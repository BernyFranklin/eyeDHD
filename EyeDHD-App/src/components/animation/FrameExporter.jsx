 ...
 */
export default function FrameExporter({ csvData, onProgress, onExportComplete, isExporting = false, fileName }) {
  ...
  // Save the video file
  const saveVideoFile = async (blob, extension = 'mp4') => {
    try {
  ...
  const startAnimation = () => {
    ...
    console.log(`Processing CSV with ${csvData.length} total frames, skipping every ${frameSkip} frames for ${playbackFps}fps playback`);
        ...
        if (nextIndex >= csvData.length) {
          ...
          setTimeout(() => {
            if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
              console.log(`Recording complete. Processed ${csvData.length} total frames.`);
              mediaRecorderRef.current.stop();
            }
          }, 500);
