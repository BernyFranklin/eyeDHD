 ...
 */
export default function ExportManager({ csvData, fileName, onExportComplete }) {
    ...
    try {
      ...
      do {
        ...
        if (batch && batch.length > 0) {
          ...
          batchCount++;
          setExportStatus(`Loading dataset... ${allData.length.toLocaleString()} rows loaded`);
        }
  ...
  const cancelExport = () => {
    ...
    const totalDuration = exportStartTimeRef.current ? (endTime - exportStartTimeRef.current) / 1000 : 0;
    ...
    console.log(`⏱️  Time elapsed before cancellation: ${Math.floor(totalDuration / 60)}m ${Math.round(totalDuration % 60)}s`);
    
    exportStartTimeRef.current = null;
  };

  const handleProgress = (progress, frame, total) => {
    setExportProgress(progress);
    setCurrentFrame(frame);
    setTotalFrames(total);
    setExportStatus(`Recording animation: Frame ${frame.toLocaleString()} of ${total.toLocaleString()} (${Math.round(progress)}%)`);
  };
  ...
  const handleExportComplete = (result) => {
    ...
    if (result.success) {
      ...
      console.log(`⏱️  Total export time: ${Math.floor(totalDuration / 60)}m ${Math.round(totalDuration % 60)}s`);
      console.log(`📊 Processed ${result.frameCount?.toLocaleString() || 'Unknown'} frames`);
      ...
      console.log(`⏱️  Time elapsed before failure: ${Math.floor(totalDuration / 60)}m ${Math.round(totalDuration % 60)}s`);
    }
      ...
      {isExporting && !isLoadingData && (
        <div>
          ...
          <div style={styles.stats}>
            Frames processed: {currentFrame.toLocaleString()} / {totalFrames.toLocaleString()}
          </div>
        </div>
      )}
