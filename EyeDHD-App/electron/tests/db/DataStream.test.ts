...
describe('Database: DataStream (batched)', () => {
  it('1) Increments progress rows by batch size', async () => {
    const batches: Batch[] = [
      [null as unknown as DataType, null as unknown as DataType],
  ...
  it('2) Treats undefined batch as empty without crashing', async () => {
    const iterator = {
      async next(): Promise<IteratorResult<Batch>> {
        return { done: false, value: undefined as unknown as Batch };
      }
    } as AsyncIterator<Batch>;
  ...
  it('3) Calls iterator.return() on close()', async () => {
    const batches: Batch[] = [[null as unknown as DataType]];
  ...
  it('4) Close is idempotent', async () => {
    const iterator = makeBatchIteratorWithReturn([]);
    const stream = new DataStream('Metadata', iterator);
