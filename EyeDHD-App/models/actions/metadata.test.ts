...
test.concurrent('files create', async ({ db, expect }: { db: Database; expect: any }) => {
  const expected = {
    id: 4,
    name: 'newData.csv',
    path: '../newData.csv',
...
test.concurrent('files read', async ({ db, expect }: { db: Database; expect: any }) => {
  const expected = {
    id: 2,
    name: 'test2.csv',
    path: 'test2.csv',
...
// test.concurrent('files update', async ({ db, expect }: { db: Database; expect: any }) => {
//   const expected = {
//     id: 2,
//     name: 'test2.csv',
//     path: 'test2.csv',
